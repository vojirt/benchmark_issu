...
class DatasetIDDFull(DatasetBase):
    ...
    def get_frame(self, key, *channels):
        ...
        if sem_gt is not None:
            ...
            label[mask_usual] = sem_gt[mask_usual]
            label[self.mask_from_label_range(sem_gt, self.cfg.classes.anomaly)] = max(self.cfg.classes.usual) + 1
...
class DatasetIDDFullAnomalyTrack(DatasetIDDFull):
    CLASS_IDS = dict(
        usual = [0, 18],
        anomaly = 254,
        ignore = 255,
    ...
    configs = [
        ...
        dict(
            # static
            name = 'IDDAnomalyFullTrack-static',
            scenes = {'static'},
            expected_length = 980,
            **DEFAULTS,
        ),
        dict(
            # temporal
            name = 'IDDAnomalyFullTrack-temporal',
            scenes ={'temporal'},
